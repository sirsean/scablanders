...
export interface RmwOptions<K extends SliceKey, TResult = unknown> {
	...
	broadcast?: (prev: Pick<SliceMap, K>, next: Pick<SliceMap, K>, changed: K[]) => BroadcastPlan | undefined;
...
export async function rmw<K extends SliceKey, TResult = unknown>(
	...
	if (
		(mutateResult && typeof mutateResult === 'object' && 'broadcast' in (mutateResult as any)) ||
		'event' in (mutateResult as any) ||
		'result' in (mutateResult as any)
	) {
	...
	await Promise.all(changed.map((key) => storage.put(key, draft[key] as any)));

	// 6) Build broadcast plan (override > custom mapper > default)
	const explicitPlan =
		overridePlan ?? options.broadcast?.(prev, draft, changed) ?? defaultBroadcastMapping(prev as any, draft as any, changed as SliceKey[]);
