...
// POST /api/missions/reconcile-vehicles - Reset any vehicles stuck on_mission without an active mission
missions.post('/reconcile-vehicles', requireAuth, async (c) => {
	try {
		const playerAddress = c.get('playerAddress')!;
		...
		const result = await (gameStub as any).reconcileVehicleStatusesForPlayer(playerAddress);
		return c.json({ success: true, ...result });
	} catch (error) {
		console.error('Reconcile vehicles error:', error);
		return c.json({ error: 'Failed to reconcile vehicles' }, 500);
	}
});
...
missions.post('/complete-oldest', requireAuth, async (c) => {
	try {
		...
		// Find oldest mission
		const oldestMission = activeMissions.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime())[0];
